...
There are a total of four coefficients (including the y-intercept).
    
    
##### **(c)** How many coefficients are estimated for the `season` variable?
  
There are three coefficients estimated for the season variable.
    
    
##### **(d)** Interpret the coefficients of `season` in the model.
   
The coefficients for each season represent how many bike rentals there are in a given day in the season above the intercept of 2604.
...
cnt_reg <- lm(cnt ~ temp + atemp + mnth + hum + windspeed, bikes)
```

##### **(b)** Interpret the coefficients of `temp`, `mnth` and `hum` in the model.

The coefficient of temp is the increase in the number of rentals* corresponding with a 1 degree Celsius rise in temperature. The coefficient of mnth is the increase in the number of rentals* based on the month (presumably going in a linear fashion). The coefficient of hum represents the decrease in the number of rentals* that corresponds with an percentage point increase in humidity.

*above/below the y-intercept
    
##### **(c)** Which predictors are associated with increased ridership?  Which predictors are associated with decreased ridership?
  
Temp, atemp, and mnth are associated with an increase in ridership. Hum and windspeed are associated with a decrease in ridership.
...
summary(cnt_reg)
```
The intercept, mnth, hum, and windspeed are significant at the alpha = 0.05 level.
...
```{r}
bikes_varnames <- c("temp", "atemp", "mnth", "hum", "windspeed")
pairs(bikes[,bikes_varnames])
```
...
```{r}
round(cor(bikes[,bikes_varnames]), 3)
```
Interestingly enough, temperature and feels-like temperature are highly correlated. I suppose this makes sense, as although it may feel different than the actual temperature, the difference is usually not by much.


##### **(c)** Refit your regression model, but this time **omit** the `temp` variable.  Display the coefficients table for this model.

```{r}
cntreg_2 <- lm(cnt ~ atemp + mnth + hum + windspeed, bikes)
```
...
summary(cntreg_2)
```
In the new model, the coefficient of atemp is 108.21. The difference between the value in this model and in the previous model is greater than 40, which is quite interesting. Unlike the first model, though, the coefficient for atemp is indeed signficantly significant at the alpha = 0.05 level. For that matter, so are the rest of the variables.
